...
Variables (Alpha : Set) (state : Set).

Notation Stream := (stream state) (only parsing).
Notation State_formula := (state_formula state) (only parsing).

Lemma wf_leadsto :
 forall (meas : state -> Alpha -> Prop) (r : relation Alpha)
   (A B C : state_formula state),
 well_founded r ->
...
intros mes r A B C H_wf str; unfold leads_to_via in |- *;
 unfold implies in |- *; intros H h; elim h; clear h.
...
cut
 (always
    (fun str : stream state =>
     state2stream_formula (fun s : state => A s /\ mes s v0) str ->
     until (state2stream_formula B)
       (state2stream_formula
          (fun s : state =>
           A s /\ (exists t : Alpha, mes s t /\ r t v0) \/ C s)) str) str);
 auto.
...
clear H1 H2 s0 str0; unfold state2stream_formula in H0;
 cut
  (until (fun str : stream state => B (head_str str))
     (fun str : stream state =>
      A (head_str str) /\ (exists t : Alpha, mes (head_str str) t /\ r t v0) \/
...
clear H_rec H_acc H_wf H A_head H_meas H_until str;
 intros s str H_B H_until H_rec H_always; constructor 2; 
 auto.
apply H_rec.
intro v;
 cut
  (always
     (fun str : stream state =>
      state2stream_formula (fun s : state => A s /\ mes s v) str ->
      until (state2stream_formula B)
        (state2stream_formula
           (fun s : state =>
            A s /\ (exists t : Alpha, mes s t /\ r t v) \/ C s)) str)
     (cons_str s str)); auto. 
...
Lemma wf_leadsto_rule :
 forall (mes : state -> Alpha -> Prop) (r : relation Alpha)
   (A B C : state_formula state),
 well_founded r ->
 forall str : stream state,
 (forall v : Alpha,
  leads_to_via (state2stream_formula (fun s : state => A s /\ mes s v))
    (state2stream_formula B)
    (state2stream_formula
       (fun s : state => A s /\ (exists t : Alpha, mes s t /\ r t v) \/ C s))
    str) ->
 leads_to_via
   (state2stream_formula
      (fun s : state => A s /\ (exists t : Alpha, mes s t)))
   (state2stream_formula B) (state2stream_formula C) str.

intros mes r A B C wf_op; cofix wf_leadsto_rule.
